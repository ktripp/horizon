...
horizon.projects = {
  ...
  **/
  ...
  list_filtering: function () {
    ...
    // remove filtering on click of 'x' button, disable 'x' button if no input
    $("a.remove_filter").attr('disabled', 'disabled');
    $("a.remove_filter").on('click', function() {
      if (!$(this).attr('disabled')) {
        $(this).attr('disabled', 'disabled');
        $(this).next().val(horizon.projects.filter_btn_text);
  
        var filter = $(this).next().attr('id');
        var rows = $(this).parents(".project_membership").children().children().children("ul." + filter).children("ul");
        if ($(rows).length > 0) {
          $(rows).show();
          $(this).parents(".project_membership").children(".filterable").children().children("#no_" + filter).hide();
        }
